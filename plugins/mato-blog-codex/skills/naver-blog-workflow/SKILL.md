...
- Use this plugin's `profiles.py` for profile creation and login checking, and `upload.py` for upload. The bundled persistent-profile runtime preserves the normal Naver login inside the same numbered local profile. It never connects to Mato Helper or `google-blog-auto` at runtime.
